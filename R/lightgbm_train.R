...
LightgbmTrain <- R6::R6Class(
  ...
  public = list(
    ...
    # define methods
    #' @description The initializer function.
    ...
    #'   training.
    #' @param split_seed A integer (defaul: NULL). Please use this argument in
    ...
    data_preprocessing = function(validation_split = 0.7, split_seed = NULL) {
      ...
      if (private$validation_split < 1) {
        ...
        train_index <- caret::createDataPartition(
          ...
          times = 1,
          p = private$validation_split
        )[[1]]

        # validation index
        valid_index <- setdiff(seq_len(nrow(self$dataset)), train_index)
    ...
    #'
    #' @details Creates a confusion matrix by using the R package `caret`.
    #'
    #' @param y_true A vector holding the ground truth of the predicted data.
    #'
    #' @seealso \code{\link[caret]{confusionMatrix}}
    #'
    confusion_matrix = function(y_true) {
      stopifnot(!is.null(private$classes))

      reference <- private$transform_target(y_true)
