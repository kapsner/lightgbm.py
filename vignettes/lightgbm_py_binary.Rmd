...
Make sure, the reticulate package is configured properly on your system (reticulate version >= 1.13.0.9006) and is pointing to a python environment. If not, you can e.g. install `miniconda`:
...
This transformation of the test set's target variable is not necessary when using the `revalue` argument of the learner's `predict` function. The value mappings are stored in the learner's object `value_mapping`:
